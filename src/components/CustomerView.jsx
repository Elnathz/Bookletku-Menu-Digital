...
export function CustomerView({ items, settings, onIncrementViews, onBack }) {
  ...
  const generateWhatsAppMessage = () => {
    ...
    // Header
    lines.push(lang === "id" ? "=== PESANAN BARU ===" : "=== NEW ORDER ===");
    lines.push("");
    ...
    if (customerName) {
      lines.push(
        lang === "id" ? "Nama: " + customerName : "Name: " + customerName
      );
    }
    lines.push(
      lang === "id"
    ...
    if (orderType === "dine-in" && tableNumber) {
      lines.push(
        lang === "id" ? "No. Meja: " + tableNumber : "Table No: " + tableNumber
      );
    }
    lines.push("");
    ...
    lines.push(
      lang === "id"
        ? "TOTAL: Rp " + formatPrice(totalPrice) + " (" + totalItems + " item)"
        : "TOTAL: Rp " + formatPrice(totalPrice) + " (" + totalItems + " items)"
    );
    lines.push("-------------------");

    if (customerNote) {
      lines.push("");
      lines.push(lang === "id" ? "Catatan Tambahan:" : "Additional Notes:");
    ...
    <div className="min-h-screen bg-gray-50 pb-32">
      ...
      {cart.length > 0 && !showCart && (
        <button
          ...
          className="fixed bottom-6 right-6 bg-[#666fb8]  text-white p-4 rounded-full shadow-lg hover:bg-[#666fb8]  transition-all z-40"
        >
          ...
          >
              ...
              {cart.map((item) => (
                <div key={item.id} className="bg-gray-50 rounded-xl p-3">
                  ...
                  <input
                    ...
                    className="w-full mt-2 px-3 py-1.5 text-xs border rounded-lg outline-none focus:ring-1 focus:ring-[#666fb8]"
                  />
                </div>
              ))}
            </div>
            ...
            {showOrderForm && (
              <div className="border-t p-4 bg-gray-50 space-y-3">
                ...
                <input
                  ...
                  className="w-full px-3 py-2 text-sm border rounded-lg outline-none focus:ring-1 focus:ring-[#666fb8]"
                />
                ...
                {orderType === "dine-in" && (
                  <input
                    ...
                    placeholder={lang === "id" ? "Nomor Meja" : "Table Number"}
                    className="w-full px-3 py-2 text-sm border rounded-lg outline-none focus:ring-1 focus:ring-[#666fb8]"
                  />
                )}
                <textarea
                  ...
                  rows={2}
                  className="w-full px-3 py-2 text-sm border rounded-lg outline-none focus:ring-1 focus:ring-[#666fb8] resize-none"
                />
              </div>
            )}
            ...
            <div className="sticky bottom-0 bg-white border-t p-4 space-y-3">
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-2 bg-[#666fb8]  text-white py-3 rounded-xl font-medium hover:bg-[#666fb8]  transition-colors"
              >
